#import "XObservedObject.h"

@implementation XObserverInfo
...
- (BOOL)isEqual:(XObserverInfo *)info {
    if (info == self) {
        return YES;
    }
...
@implementation XObservedObject {
...
- (void)addObserver:(NSObject *)observer forKeyPath:(NSString *)keyPath options:(NSKeyValueObservingOptions)options context:(void *)context {
    XObserverInfo *infoToAdd = [[XObserverInfo alloc] initWithObserver:observer keyPath:keyPath];

    for (XObserverInfo *info in _observerInfos) {
...
- (void)removeObserver:(id)observer {
    ...
    for (XObserverInfo *info in auxSet) {
